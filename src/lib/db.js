...
// ===== Helper: apply dynamic overdue status =====
function applyDynamicStatus(invoice) {
...
export async function createUser(name, role = 'staff', extraFields = {}) {
  const { data, error } = await supabase
    .from('users')
    .insert({
      name,
      ...
      birth_date: extraFields.birth_date || null,
      address_house_no: extraFields.address_house_no || null,
      address_subdistrict: extraFields.address_subdistrict || null,
      address_district: extraFields.address_district || null,
      address_province: extraFields.address_province || null,
      address_zipcode: extraFields.address_zipcode || null,
    })
    .select()
    .single()
  if (error) return handleError(error)
...
export async function getCustomerInsight(customerId) {
  ...
  const invoices = rawInvoices.map(applyDynamicStatus)
  ...
  if (invoiceIds.length > 0) {
    const { data: items, error: itemsErr } = await supabase
      .from('invoice_items')
      .select('product_name, qty, total_price, products(image_url)')
    ...
    if (!itemsErr) {
      ...
      items.forEach(item => {
        if (!productMap[item.product_name]) {
          productMap[item.product_name] = { qty: 0, total: 0, image_url: item.products?.image_url || null }
        }
        productMap[item.product_name].qty += item.qty
        productMap[item.product_name].total += Number(item.total_price)
        // เก็บ image_url ถ้ายังไม่มี
        if (!productMap[item.product_name].image_url && item.products?.image_url) {
          productMap[item.product_name].image_url = item.products.image_url
        }
      })
...
export async function getInvoice(id) {
  const { data, error } = await supabase
    ...
    .single()
  if (error) return handleError(error)
  return applyDynamicStatus(data)
}
...
export async function getDashboardData(filters = {}) {
  let query = supabase
    .from('invoices')
    .select('*, customers(name, image_url)')
    .is('deleted_at', null)
  ...
  invoices.forEach(inv => {
    ...
    const image = inv.customers?.image_url || null
    if (!customerMap[id]) customerMap[id] = { id, name, total: 0, image }
    customerMap[id].total += Number(inv.total)
  })
  ...
  if (invoiceIds.length > 0) {
    const { data: items } = await supabase
      .from('invoice_items')
      .select('product_id, product_name, qty, products(image_url)')
      .in('invoice_id', invoiceIds)
    
    const productMap = {}
    ;(items || []).forEach(item => {
      const id = item.product_id || item.product_name || `custom_${Math.random()}`
      const name = item.product_name || 'ไม่ระบุ'
      const image = item.products?.image_url || null
      if (!productMap[id]) productMap[id] = { id, name, qty: 0, image }
      productMap[id].qty += item.qty
    })
...
export async function saveDailySales(date, entries) {
  // entries: Array of { product_id, category, platform, qty }
  
  for (const entry of entries) {
    const { product_id, category, platform, qty } = entry
    
    // 1. Get existing record to calculate stock difference
    ...
    // 2. Update Daily Sales Record (Upsert)
    const { error: upsertErr } = await supabase
      ...
      .upsert({
        ...
        platform,
        qty
      }, { onConflict: 'sale_date, product_id, category, platform' })

    if (upsertErr) {
      handleError(upsertErr, 'ไม่สามารถบันทึกยอดขายรายวันได้')
      continue
    }

    // 3. Deduct Stock from Products table
    // Note: Since it's a "sale", a positive qty in daily_sales means stock goes DOWN.
    // So we subtract 'diff' from stock.
    const { data: product } = await supabase
      .from('products')
    ...
    if (product) {
      const newStock = (product.stock_qty || 0) - diff
      ...
      if (stockErr) {
        handleError(stockErr, `ไม่สามารถตัดสต็อกสินค้า ${product.name} ได้`)
      } else {
        // 4. Record Movement
        await supabase.from('stock_movements').insert({
          product_id,
          type: diff > 0 ? 'out' : 'in', // If diff > 0 (more sales), it's 'out'. If diff < 0 (reduced sales), it's 'in'.
          qty: Math.abs(diff),
          note: `บันทึกยอดขายรายวัน (${category}/${platform || 'claim'}) วันที่ ${date}`
        })
      }
    }
  }
...
export async function getDueSoonInvoices() {
  const todayStr = new Date().toLocaleDateString('sv-SE', { timeZone: 'Asia/Bangkok' })
  const in7Days = new Date()
  in7Days.setDate(in7Days.getDate() + 7)
  const in7DaysStr = in7Days.toLocaleDateString('sv-SE', { timeZone: 'Asia/Bangkok' })
